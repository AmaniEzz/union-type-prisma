...
We also included a complete example [here](www.) on how to use **Multiple Foreign keys approach** to work around the Prisma lack of union type support with a **GraphQL** layer to that contains actual union types.
...
You can find the complete example [here](www.) on how to use **Single Polymorphic Association Table** approach to work with union types in TypeScript when using Prisma to query a relational databasew. The example also include **GraphQL** schema that contain union types and resolvers to resolve each type.
